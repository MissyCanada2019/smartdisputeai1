...
import { disputeCategories, provinces } from "@shared/schema";
...
import type { ResourceCategory, ResourceSubcategory } from "@shared/schema";
        ...
        <div className="mb-8">
          ...
          {categoriesLoading ? (
            ...
            <Tabs onValueChange={setActiveCategory} defaultValue="">
              ...
              {/* Category tabs */}
              {!categoriesLoading && categories?.map((category: ResourceCategory) => (
                <TabsContent key={category.id} value={category.id.toString()} className="mt-6">
                  ...
                  {resourcesLoading ? (
                    ...
                    resources?.filter((r: Resource) => r.categoryId === category.id).length ? (
                      <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                        {resources
                          .filter((r: Resource) => r.categoryId === category.id)
                      ...
                      <div className="text-center py-10 bg-gray-50 rounded-lg border border-dashed">
                        ...
                        <p className="text-gray-500 text-sm mt-1">Try changing your province filter or check back later.</p>
                      </div>
                    )
                  )}
                </TabsContent>
