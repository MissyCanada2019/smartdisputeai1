/**
 * Test script for Anthropic service using the latest SDK and Claude 3.7 Sonnet model
 */
...
// Constants
const DEFAULT_MODEL = 'claude-3-7-sonnet-20250219'; // the newest Anthropic model is "claude-3-7-sonnet-20250219" which was released February 24, 2025
...
function extractTextFromContent(content: any[]): string {
  ...
  // Find and concatenate all text blocks
  return content
...
async function testBasicTextGeneration() {
  try {
    console.log('\n🔍 Testing basic text generation with Claude...');
    ...
    const response = await anthropic.messages.create({
      ...
      system: 'You are a helpful assistant with expertise in legal affairs and social justice.'
    });
...
/**
 * Test JSON structured output with Claude
 */
async function testJsonResponse() {
  try {
    console.log('\n🔍 Testing JSON structured output with Claude...');
    ...
    const response = await anthropic.messages.create({
      ...
      max_tokens: 1000,
      temperature: 0.3,
      system: 'You are a legal assistant AI that provides information about tenant rights in Ontario, Canada. Respond with structured JSON data only.',
      messages: [
        { role: 'user', content: `Analyze this tenant situation and provide structured advice in JSON format: ${legalSituation}` }
      ]
    });
    ...
    let jsonContent;
    try {
      // Find JSON content between markers or try to parse the entire response
      const jsonMatch = textContent.match(/```json\s*([\s\S]*?)\s*```/) || 
                       textContent.match(/\{[\s\S]*\}/);
                       
      const jsonString = jsonMatch ? jsonMatch[0].replace(/```json|```/g, '').trim() : textContent;
...
async function testDocumentAnalysis() {
  try {
    console.log('\n🔍 Testing legal document analysis with Claude...');
    ...
    const response = await anthropic.messages.create({
      ...
      `,
      messages: [
        { role: 'user', content: `Analyze this legal document and provide a structured analysis in JSON format:
    ...
    console.log('\n✅ Claude Document Analysis Response:');
...
async function testImageAnalysis() {
  try {
    ...
    const visionContent = extractTextFromContent(response.content);
...
async function runTests() {
  console.log('\n🚀 Starting Anthropic Claude service tests...');
